...
pub fn asteroid_movement(world: &mut World, dt: f32){
    ...
    world.query_mut::<With<(Entity, &Life, &mut Position, &Direction, &Size, &mut Rotation, &Speed, &Sides), &Asteroid>>()
    .into_iter()
    ...
    world.query_mut::<With<(&mut Position, &Direction, &Sides, &mut Rotation), &Fragment>>()
    .into_iter()
    .for_each(|(pos, dir, sides, rot)|{
        ...
        draw_poly_lines(pos.0.x, pos.0.y, sides.0, FRAGMENT_SIZE, rot.0, 8.5, ASTEROID_COLOR_1);
    });

    delete_pool.iter().for_each(|e| {let _ = world.despawn(*e);});
