...
pub fn inputs_player(world: &mut World, dt: f32) {
    world
        .query_mut::<With<(&mut OnMove, &mut Position, &mut Direction, &mut Rotation), &Player>>()
        .into_iter()
    ...
    world
        .query::<(&Player, &Position, &Size, &Rotation)>()
        .iter()
        .for_each(|(_player, pos, size, rot)| {
            ...
            let vec_a = vec2(pos.0.x + h * sen_a, pos.0.y - h * cos_a);
